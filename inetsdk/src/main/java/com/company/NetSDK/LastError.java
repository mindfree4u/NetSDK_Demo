...
public enum LastError {
    ...
    // 当前设备未支持该功能，或是功能在当前SDK中被裁剪
    NET_NOT_SUPPORTED((0x80000000 | (23)),"当前设备未支持该功能，或是该功能在当前SDK中被裁剪","The current device does not support this function, or this function is cut in the current SDK"),
    ...
    NET_ERROR_LOGOPEN_DISABLE((0x80000000 | (540)),"存在日志配置文件，以日志打印配置文件为准，日志打印接口失效","LogCfg is exist,the CLIENT_LogOpen interface is disable."),
    ...
    // 密码重置功能已关闭
    NET_ERROR_PWD_RESET_DISABLE((0x80000000 | (1025)),"密码重置功能已关闭","pwd reset Unabled"),
    ...
    // 批量添加人脸停止
    NET_ERROR_FACE_RECOGNITION_SERVER_MULTI_APPEND_STOUP((0x80000000 | (1049)),"批量添加人脸停止","Target recognition server multi append stop"),

    // 批量添加人脸失败
    NET_ERROR_FACE_RECOGNITION_SERVER_MULTI_APPEND_ERROR((0x80000000 | (1050)),"批量添加人脸失败","Target recognition server multi append error"),
    ...
    // 人员数量超过限制
    NET_ERROR_FACE_RECOGNITION_SERVER_FACE_MAX_NUM((0x80000000 | (1063)),"人员数量超过限制","Target recognition server face max num"),
    ...
    // 超出人脸库空间大小限制
    NET_ERROR_FACE_RECOGNITION_SERVER_GROUP_SPACE_EXCEED((0x80000000 | (1074)),"超出人脸库空间大小限制","Target recognition server group space exceed"),

    // 超出人脸库图片数量限制
    NET_ERROR_FACE_RECOGNITION_SERVER_GROUP_PIC_COUNT_EXCEED((0x80000000 | (1075)),"超出人脸库图片数量限制","Target recognition server group pic count exceed"),

    // 人脸库不存在
    NET_ERROR_FACE_RECOGNITION_SERVER_GROUP_NOT_FOUND((0x80000000 | (1076)),"人脸库不存在","Target recognition server group not found"),

    // 查询原人脸库数据结果无效
    NET_ERROR_FACE_RECOGNITION_SERVER_FIND_RECORDS_ERROR((0x80000000 | (1077)),"查询原人脸库数据结果无效","Target recognition server find record error"),

    // 删除原人脸库数据失败
    NET_ERROR_FACE_RECOGNITION_SERVER_DELETE_PERSON_ERROR((0x80000000 | (1078)),"删除原人脸库数据失败","Target recognition server delete person error"),

    // 删除人脸库失败
    NET_ERROR_FACE_RECOGNITION_SERVER_DELETE_GROUP_ERROR((0x80000000 | (1079)),"删除人脸库失败","Target recognition server delete group error"),

    // 命名格式错误
    NET_ERROR_FACE_RECOGNITION_SERVER_NAME_FORMAT_ERROR((0x80000000 | (1080)),"命名格式错误","Target recognition server name format error"),

    // 图片保存路径未设置
    NET_ERROR_FACE_RECOGNITION_SERVER_FILEPATH_NOT_SET((0x80000000 | (1081)),"图片保存路径未设置","Target recognition server file path not set"),
    ...
    NET_ERROR_SECURITY_ERROR_SUPPORT_MULT((0x80000000 | (1105)),"校验请求安全码失败,可使用私有渠道APP、configtool工具重置密码","check request security failed, using private APP or configtool reset password"),
    ...
    // 调用私有加密库产生安全码失败
    NET_ERROR_SECURITY_GENERATE_SAFE_CODE((0x80000000 | (1108)),"调用私有加密库产生安全码失败","generate safe code failed"),
    ...
    private LastError(int givenValue, String ChnNote, String EngNote) {
    ...
    public static String getChnNoteByValue(int givenValue) {
        for (LastError enumType : LastError.values()) {
