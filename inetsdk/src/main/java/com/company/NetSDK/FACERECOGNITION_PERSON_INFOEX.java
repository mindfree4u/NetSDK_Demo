...
public class FACERECOGNITION_PERSON_INFOEX implements Serializable {
     ...
     * \endif
     */
    public NET_PERSON_FEATURE_VALUE_INFO					stuPersonFeatureValue = new NET_PERSON_FEATURE_VALUE_INFO();
     ...
     * \endif
     */
    public NET_PERSON_FREQUENCY_INFO					stuFrequencyInfo = new NET_PERSON_FREQUENCY_INFO();

    /**
     * \if ENGLISH_LANG
     *  The platform uniquely identifies the face field, which is different from UID (customized by Evergrande Smart Community). According to the capability of faceRecognitionServer.getCaps, IVSS supports UUID when the value of SupportIDFromServer is true.
     * \else
     *  平台唯一标识人脸字段，区别于UID（恒大智慧社区使用）,IVSS根据faceRecognitionServer.getCaps能力，SupportIDFromServer值为true时，支持UUID有效
     * \endif
     */
