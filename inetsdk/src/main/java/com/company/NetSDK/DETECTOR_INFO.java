...
public class DETECTOR_INFO implements Serializable {
	...
	/**
	 * \if ENGLISH_LANG
	 * DrivingDirection: "Approach" means driving direction,where the car is more near;"Leave"-means where if mor far to the car,The scend and third param means the location of the driving direction, Overspeed  percentage, Snapshot taken when exceed speed limit
	 * \else
	 * "DrivingDirection" : 行驶方向, "Approach"-上行，即车辆离设备部署点越来越近；"Leave"-下行，即车辆离设备部署点越来越远，第二和第三个参数分别代表上行和,下行的两个地点，UTF-8编码
	 * \endif
	 */
